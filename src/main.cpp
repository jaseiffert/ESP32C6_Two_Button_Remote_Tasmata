...
# ################################################################################################ #
# File: main.cpp                                                                                   #
# Project: XIAO Two Button Remote                                                                  #
# Created Date: Friday, October 10th 2025, 11:27:20 am                                             #
...
#define BUTTON_LEFT 5 // Use D3 if your okay with only one wakeup button. Easier to solder than the MTDI under the board.
...
const char* serverButtonLeft = LEFT_BUTTON_SERVER_IP;
const char* serverButtonRight = RIGHT_BUTTON_SERVER_IP;


unsigned long Left_button_time = 0;
...
void setup() {
    ...
    /*
        External Antenna
        GPIO14 is used to select between using the built-in antenna or an external antenna.
        Before that, you need to set GPIO3 low level to turn on this function. If GPIO14 is set low level, it uses the built-in antenna;
        if it set to high level, it uses the external antenna. Default is low level.
    */
    /*
        pinMode(3, OUTPUT);
    ...
    */
    ...
    pinMode(BATTERY_VOLTAGE, INPUT); // ADC
    ...
    if (esp_deep_sleep_enable_gpio_wakeup(BIT(BUTTON_LEFT), ESP_GPIO_WAKEUP_GPIO_HIGH) == ESP_OK && esp_deep_sleep_enable_gpio_wakeup(BIT(BUTTON_RIGHT), ESP_GPIO_WAKEUP_GPIO_HIGH) == ESP_OK) {
...
void loop() {
    ...
    if (buttonStateLeft == HIGH) {
        ...
        String response = "";
        if (Left_button_time - last_button_time > 250) {
            response = httpGETRequest(serverButtonLeft);
            debugpln(response);
            ...
            last_button_time = Left_button_time;
        }
    ...
    if (buttonStateRight == HIGH) {
        ...
        String response = "";
        if (right_button_time - last_button_time > 250) {
            debugpln("Lights On.");
            response = httpGETRequest(serverButtonRight);
            debugpln(response);
            if (response.substring(10, 12) == "ON") {
                digitalWrite(LED_RED, LOW);
                digitalWrite(LED_GREEN, HIGH);
            ...
            last_button_time = right_button_time;
            delay(1000);
            digitalWrite(LED_RED, LOW);
            digitalWrite(LED_GREEN, LOW);
        }
    ...
    if (sleep_time - last_sleep_time > ((SLEEP_DELAY_S * mS_TO_S_FACTOR))) {
        checkBatteryVoltage();
        debugp("Sleep Time - Last Sleep Time = ");
        debugpln(sleep_time - last_sleep_time);
        debugp("Time to sleep: ");
        debugpln(String(SLEEP_DELAY_S * mS_TO_S_FACTOR));
        last_sleep_time = sleep_time;
...
void checkBatteryVoltage() {
    ...
    if (!fiveVoltPin) {
        ...
        uint32_t Vbatt = 0;
        for (int i = 0; i < 16; i++) {
            Vbatt = Vbatt + analogReadMilliVolts(BATTERY_VOLTAGE); // ADC with correction
        }
